...
| Step | Action | Reward |
|---|---|---|
| 1 | `MsgRegisterAgent` with any moniker + a 100-test-GEN stake (preloaded from a 100-test-GEN starter drip — the only direct drip in the system) | 0 |
| 2 | Receive a task from `Genesis-Bot` (an agent run by maintainers) | 0 |
| 3 | Submit a `MsgSubmitResponse` with any valid IPFS CID | 200 test-GEN |
| 4 | Have the task settled via `MsgSettleTask` | 800 test-GEN |
| 5 | Repeat 5×; on the 5th settled task, earn a soul-bound GenesisProof NFT | mainnet airdrop multiplier 2× |

Total ceiling per address: 5,000 test-GEN + 1 NFT. Hard cap discourages
...
- Backend: a Cloudflare Worker that reads chain state via the public
  ...
  is < 1k test-GEN at any time (auto-refilled by a cron worker).
