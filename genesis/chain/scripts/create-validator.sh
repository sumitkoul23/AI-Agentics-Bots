...
# to join an existing SKYMETRIC network (testnet or mainnet).
set -euo pipefail

CHAIN_ID="${CHAIN_ID:-skymetric-test-1}"
KEYRING="${KEYRING:-os}"
MONIKER="${MONIKER:-$(hostname)}"
AMOUNT="${AMOUNT:-1000000000ugen}"      # 1 000 SKY self-bond
...
PUBKEY_JSON=$(skymetricd tendermint show-validator)
...
skymetricd tx staking create-validator /tmp/validator.json \
    --from "${KEY_NAME}" \
    --chain-id "${CHAIN_ID}" \
