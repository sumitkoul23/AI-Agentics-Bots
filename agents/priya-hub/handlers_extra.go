...
// ── Media Finder ──────────────────────────────────────────────────────────────
// Helps locate publicly available videos and pictures of a person using three
// signals: the image/video name, a visual description (the "demo": appearance,
// clothing, age, distinguishing features), and a location.

func handleMediaFinder(input string, mem *Memory) string {
	lower := strings.ToLower(input)
	switch {
	case strings.Contains(lower, "reverse image search") || strings.Contains(lower, "face search"):
		return "Reverse-image and face search: start with a clear, front-facing reference photo of the person. Run it through Google Lens / Google Images (images.google.com → camera icon), Yandex Images (strongest for faces), Bing Visual Search, TinEye (finds exact copies and where they were first posted), and PimEyes (face-recognition across the public web). Crop tightly to the face for face search, but keep the full frame for context search. Cross-reference hits across at least two engines before trusting a match. Use only for people who've consented or for legitimate, lawful purposes."
	case strings.Contains(lower, "name") && (strings.Contains(lower, "video") || strings.Contains(lower, "photo") || strings.Contains(lower, "picture") || strings.Contains(lower, "image")):
		return "Searching by name: put the full name in quotes to force an exact match — \"Jane Smith\" — then layer qualifiers: \"Jane Smith\" Austin photographer. Search each platform directly: Google Images, YouTube, Instagram, TikTok, Facebook, Flickr, and X/Twitter media tabs. Try name variations (nicknames, maiden names, middle initials) and the name in the local language if relevant. Add a year (\"Jane Smith\" 2024) or an event name to narrow noisy results. For common names, the description and location below are what disambiguate the right person."
	case strings.Contains(lower, "demo") || strings.Contains(lower, "description") || strings.Contains(lower, "appearance") || strings.Contains(lower, "wearing") || strings.Contains(lower, "looks like"):
		return "Searching by visual description (the \"demo\"): translate the person's appearance into searchable terms — approximate age, build, hair colour/style, skin tone, glasses, facial hair, tattoos, and any distinctive clothing or accessories. Combine them: 'tall man beard glasses red jacket conference'. Use Google Images filters (type: Face, colour, time) and reverse-search any still you have. For video, scrub thumbnails on YouTube/TikTok with descriptive + location terms. The more specific and unusual the detail (a logo on a shirt, a unique tattoo), the faster you'll isolate the right person from look-alikes."
	case strings.Contains(lower, "location") || strings.Contains(lower, "where") || strings.Contains(lower, "city") || strings.Contains(lower, "near") || strings.Contains(lower, "geotag"):
		return "Searching by location: add the place at every level of specificity — venue/landmark, neighbourhood, city, region. Use Instagram and Snapchat location tags and Snap Map for geo-pinned public posts, search '<place name> <event> photos', and check Google Maps / local news galleries for event coverage. For media you already have, inspect EXIF metadata (many photos carry GPS coordinates) and look for visual geolocation clues — street signs, storefronts, license plates, architecture — then confirm in Google Street View. Combine location with the name and description to triangulate the exact person and moment."
	case strings.Contains(lower, "video") || strings.Contains(lower, "footage") || strings.Contains(lower, "reel") || strings.Contains(lower, "youtube") || strings.Contains(lower, "tiktok"):
		return "Finding video of a person: search YouTube, TikTok, Instagram Reels, Facebook, Vimeo, and X using name + description + location together. Use Google's video tab and add a date range. For a face match across video, take a sharp screenshot of a single frame and run it through reverse-image/face search — that surfaces the source clip and re-uploads. Event livestreams and local news archives are rich sources. Check the uploader's other posts and tagged accounts, then read captions and comments for additional names, dates, and venues that point to more footage."
	case strings.Contains(lower, "picture") || strings.Contains(lower, "photo") || strings.Contains(lower, "image"):
		return "Finding pictures of a person: combine the three signals — exact name in quotes, visual description, and location — across Google Images, Bing, Yandex, Instagram, Flickr, and tagged Facebook albums. If you already have one photo, reverse-search it to find every other place that image (and visually similar ones) appears. Event and venue galleries, photographer portfolios, and local press cover crowds well. Save the best reference shot and iterate: each new detail you confirm (a workplace, a friend's tagged account) becomes a fresh search term."
	default:
		return "Media Finder helps you locate publicly available videos and pictures of a person using three signals: (1) the image/video name or the person's name, (2) a visual description — the 'demo': age, build, hair, clothing, distinctive features, and (3) a location. Give me whichever you have and I'll build targeted searches across Google Images, YouTube, TikTok, Instagram, Bing, Yandex, and reverse-image/face tools, then show you how to triangulate the right match. Note: use this only for people who have consented or for lawful, legitimate purposes — respect privacy and platform terms."
	}
}
